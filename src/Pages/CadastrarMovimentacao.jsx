...
} from "../components/MovimentacaoService";
...
const MovimentoContabil = () => {
 ...
 // const history = useHistory();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await atualizarPlanoConta(dados.idMovimentoContabil, {
      ...
      const data = await visualizarMovimentoContabil();
      setPlanos(data);
      setDados({ idMovimentoContabil: "", CodigoPlano: "", Descricao: "", Tipo: "" });
      setIsEditing(false);
      setShowModal(false);
    } catch (error) {
  ...
  const handleDelete = async (idMovimentoContabil) => {
    try {
      await deletarPlanoConta(idMovimentoContabil);
      const data = await visualizarMovimentoContabil();
      setPlanos(data);
    } catch (error) {
  ...
  return (
    <div className="container mt-5">
      <h1>Planos de Contas.Cadastrar</h1>
      <Button variant="primary" onClick={() => setShowModal(true)}>
        Cadastrar Novo Plano
      </Button>
      <ul className="list-group mt-4">
        {planos.map((plano) => (
          <li key={plano.idMovimentoContabil} className="list-group-item d-flex justify-content-between align-items-center">
            {plano.idMovimentoContabil} // {plano.CodigoPlano} - {plano.Descricao} - {plano.Tipo}
            <div>
              <Button variant="warning" className="mr-2" onClick={() => handleEdit(plano)}>Editar</Button>
              <Button variant="danger" onClick={() => handleDelete(plano.idMovimentoContabil)}>Deletar</Button>
            </div>
          </li>
        ))}
      </ul>
